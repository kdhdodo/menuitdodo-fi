  ...
  function handleFile(e) {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // 고정 헤더 (발주현황 컬럼 순서)
        const FIXED = ["회사구분","업무구분","SAP PO번호","PO항목","발주일자","품목코드","품목명","규격","단위","발주수량","통화","단가","금액","작성자","저장위치","구매요청번호","구매요청항목","구매담당자","연락처담당자","상호명"];

        // 데이터 행만 추출: 숫자가 1개 이상 있는 행 = 데이터
        const data = [];
        for (let i = 0; i < raw.length; i++) {
          ...
          const hasNum = filled.some(c => typeof c === "number");
          if (!hasNum) continue; // 숫자 없으면 헤더/제목 행 → 스킵
          const row = {};
          FIXED.forEach((col, j) => { row[col] = r[j] != null ? r[j] : ""; });
          data.push(row);
        }
